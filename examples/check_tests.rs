...
fn main() {
    ...
    let mut passed = 0;
    let mut failed = 0;
    let mut failed_tests: Vec<(usize, String, String, String)> = Vec::new();
        ...
        if line.contains("example") && line.contains("````") {
            ...
            if result == html {
                ...
                if failed_tests.len() < 20 {
                    failed_tests.push((test_number, markdown, html, result));
                }
            }
        }
    ...
    if !failed_tests.is_empty() {
        println!("\n=== First {} Failed Tests ===", failed_tests.len());
        for (num, input, expected, got) in &failed_tests {
            println!("\nTest #{}", num);
            println!("Input: {:?}", input);
